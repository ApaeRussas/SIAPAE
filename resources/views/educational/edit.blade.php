    ...
    <x-table-edit 
        ...
        <div class="my-3 grid grid-cols-1 sm:grid-cols-4 gap-x-4 gap-y-3">
            ...
            <div class="col-span-1">
                ...
                <x-form.select idSelect="period" valueName="period" full>
                    ...
                    <option value="1º" {{old('period', $pedagogical->period) == '1º' ? 'selected' : ''}}>
                        1º
                    </option>
                    <option value="2º" {{old('period', $pedagogical->period) == '2º' ? 'selected' : ''}}>
                        2º
                    </option>
                </x-form.select>
        ...
        <div class="mb-3 grid grid-cols-1 sm:grid-cols-4 gap-x-4 gap-y-3">
            <div class="col-span-1 sm:col-span-2">
                ...
                <x-form.input id="school" name="school" value="{{old('school', $pedagogical->school)}}" class="w-full dark:text-gray-400"
            ...
            <div class="col-span-1">
                ...
                <x-form.input id="date_of_birth" name="date_of_birth" value="{{old('date_of_birth', \Carbon\Carbon::createFromFormat('Y-m-d', $pedagogical->student->date_of_birth)->format('d/m/Y'))}}" class="w-full sm:w-auto dark:text-gray-400" readonly
                placeholder="Data fixa"/>
            </div>
            <div class="col-span-1">
                <x-form.label for="age"> Idade <span class="text-red-700 dark:text-red-500">*</span> </x-form.label>
                <x-form.input id="age" name="age" value="{{old('age', $pedagogical->age)}}" required placeholder="Ex: 15 anos" class="w-full sm:w-48 dark:text-gray-400"/>
                @error("age")
                    <span class="text-red-600 dark:text-red-400">{{$message}}</span>
                @enderror
            </div>
        ...
        <div class="mb-3 grid grid-cols-1 sm:grid-cols-4 gap-x-4 gap-y-3">
            <div class="col-span-1">
                ...
                <x-form.input id="turn_school" name="turn_school" value="{{old('turn_school', $pedagogical->turn_school)}}" 
                    required placeholder="Ex: Manhã" class="w-full dark:text-gray-400"/>
                @error("age")
                    <span class="text-red-600 dark:text-red-400">{{$message}}</span>
                @enderror
            </div>
            <div class="col-span-1">
                ...
                <x-form.input id="grade_school" name="grade_school" value="{{old('grade_school', $pedagogical->grade_school)}}"
                    required placeholder="Ex: 1°" class="w-full dark:text-gray-400"/>
                @error("grade_school")
                    <span class="text-red-600 dark:text-red-400">{{$message}}</span>
                @enderror
            </div>
        ...
        <div>
            ...
            <x-form.textarea id="text" name="text" height="text" sizeFont="base" placeholder="Texto ........." required>
        ...
        <div class="w-full sm:w-1/2">
            ...
            <x-form.select idSelect="signature_id" valueName="signature_id" full>
                ...
                @foreach ($professors as $professor)
                    <option value="{{$professor->id}}" {{old('signature_id', $pedagogical->signature_id) == $professor->id ? 'selected' : ''}}>
                        {{$professor->name}}
                    </option>
                @endforeach
            </x-form.select>
